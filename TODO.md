...
### 2026-08-08 — Continuous implementation audit (3rd pass)

- **Note:** upstream removed the entire `functions/` directory (commit `14b865c`, 2026-08-05 — maintainers migrating CI/deploy to a new setup: Bun + Node 24, new Firebase Hosting deploy workflow, `firebase.json` removed). The server-side half of the auth hardening below was therefore dropped during rebase; only the client-side contract and the frontend fix landed.
- **Change (client-side auth contract):** `src/lib/api.ts` — `callFunction` now attaches the current user's Firebase ID token as an `Authorization: Bearer <token>` header when signed in (new `getCurrentIdToken()` helper), so any Cloud Function that verifies the caller no longer has to trust a uid from the request body (a body-supplied uid would let anyone act on another user's subscription). The uid stays in the payload only for backward compatibility. `src/lib/__tests__/api-auth.test.ts` — 2 new tests pinning the header behavior (signed-in attaches, signed-out omits).
- **Fix:** completed the half-applied AccountPage status normalization — the working tree had added a required `status` prop to `SubscriptionContent` without passing it (TS2741) and orphaned `isInGracePeriod` (TS6133), so the tree did not compile. The parent now normalizes `past_due` + expired billing period to `canceled` for both the badge and the content, so the "Payment Issue" state only renders while the Stripe grace period is actually active.
- **Audit verification:** tsc -b clean, oxlint 0/0, 361/361 unit tests passing (32 files), fallow audit/health/dead-code/dupes clean, knip clean.
- **Still blocked (3):** Lighthouse audit (needs deployment), Domain config (needs Firebase project), OpenSpec CI integration (deferred — interactive mode).
- **Flagged for maintainers (not touched):** README still documents `functions/` (setup steps, Stripe config, project tree, scripts table) and the new `deploy.yml` runs the Firebase Hosting action without a `firebase.json` — both are stale after the functions removal and should be reconciled in the migration.
